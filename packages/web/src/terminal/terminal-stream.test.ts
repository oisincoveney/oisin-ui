...
import { decodeBinaryMuxFrame, TerminalBinaryMessageType, type BinaryMuxFrame } from './binary-mux'
...
function decodeFrames(frames: Uint8Array[]): BinaryMuxFrame[] {
  return frames.map((frame) => decodeBinaryMuxFrame(frame)).filter((frame): frame is BinaryMuxFrame => frame !== null)
}
...
describe('TerminalStreamAdapter queued input', () => {
  it('queues while disconnected and flushes on attach confirmation', () => {
    ...
    let now = 0
    const adapter = new TerminalStreamAdapter(
      createTerminalStub(),
      11,
      (data) => {
